 ...
 * base class
 */
include_once XOOPS_ROOT_PATH . '/modules/sam/class/database/database.php';
 ...
 * @subpackage database
 */
class SamMySQLDatabase extends SamDatabase
{
    /**
     * Database connection
     *
    ...
    function connect($selectdb = true)
    {
        ...
        if (XOOPS_DB_PCONNECT == 1) {
            $this->conn = @mysql_pconnect($GLOBALS['samModuleConfig']['db_host'], $GLOBALS['samModuleConfig']['db_user'], $GLOBALS['samModuleConfig']['db_pass']);
        } else {
            $this->conn = @mysql_connect($GLOBALS['samModuleConfig']['db_host'], $GLOBALS['samModuleConfig']['db_user'], $GLOBALS['samModuleConfig']['db_pass']);
        }
        ...
        if ($selectdb != false) {
            if (!mysql_select_db($GLOBALS['samModuleConfig']['db_name'])) {
                $this->logger->addQuery('', $this->error(), $this->errno());
                return false;
            }
        }
        if (!isset($db_charset_set) && defined('XOOPS_DB_CHARSET') && XOOPS_DB_CHARSET) {
            $this->queryF("SET NAMES '" . $GLOBALS['samModuleConfig']['db_charset'] . "'");
        }
        $db_charset_set = 1;
    ...
    /**
     * SamMySQLDatabase::fetchObjected()
     *
     * @param mixed $result
     * @return
     */
 ...
 * @subpackage database
 */
class SamMySQLDatabaseSafe extends SamMySQLDatabase
{
    /**
     * perform a query on the database
     *
 ...
 * @subpackage database
 */
class SamMySQLDatabaseProxy extends SamMySQLDatabase
{
    /**
     * perform a query on the database
     *
