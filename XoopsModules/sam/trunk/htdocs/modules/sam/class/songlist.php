...
class SamSonglistHandler extends SamPersistableObjectHandler
{
    ...
    function getTopRequestedSongs($code = '200', $as_object = true, $id_as_key = true) {
    	$sql = 'SELECT `s`.*, count(`songID`) as `count` from `songlist` as `s` JOIN `requestlist` as `r` ON `r`.`songID` = `s`.`ID` WHERE `r`.`code` = \''.$code.'\' AND DATEDIFF(CURRENT_DATE, `r`.`t_stamp`) <= '.$GLOBALS['samModuleConfig']['request_days'].' GROUP BY `r`.`songID` LIMIT '.$GLOBALS['samModuleConfig']['top_request_count'];
    	if (!$result = $this->db->queryF($sql)) {
    		return false;
    	}
    	$ret = array();
