 ...
 */
function songinfo(songID, xoops_url, dirname)
{
	var songwin = window.open(xoops_url+"/modules/"+dirname+"/songinfo.php?songID="+songID, "songinfowin", "location=no,status=no,menubar=no,scrollbars=yes,height=400,width=650");
	songwin.focus();
}
 ...
 */
function requestPrivate(songID, xoops_url, dirname)
{
	reqwin = window.open(xoops_url+"/modules/"+dirname+"/request.php?songID="+songID, "_AR_request", "location=no,status=no,menubar=no,scrollbars=yes,resizeable=yes,height=420,width=668");
	reqwin.focus();
}
